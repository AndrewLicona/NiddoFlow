            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {accounts.map((account: any) => (
                    <Card
                        key={account.id}
                        variant="elevated"
                        className={`group hover:scale-[1.01] transition-all duration-300 border-l-4 ${account.type === "personal"
                                ? "border-l-indigo-500 border-indigo-500/10"
                                : "border-l-emerald-500 border-emerald-500/10"
                            }`}
                    >
                        <div className="flex justify-between items-start mb-6">
                            <div
                                className={`p-3 rounded-2xl shadow-sm border ${account.type === "personal"
                            ...
                            <span
                                className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${account.type === "personal"
                        ...
                        <div className="space-y-1">
                            <Typography variant="body" className="font-bold opacity-40 uppercase tracking-tighter text-[9px]">
                                {account.type === "personal" ? "Billetera" : "Cuenta de Ahorros"}
                            </Typography>
                            <Typography variant="h3" className="truncate text-foreground font-black tracking-tight">
                                {account.name}
                            </Typography>
                        </div>
